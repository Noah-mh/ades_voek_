...
import * as customerModel from "../model/customer.model.js";
...
export const processVerifyOTP = async (
  ...
  try {
    ...
    const response = await customerModel.handleVerifyOTP(
      customer_id,
      OTP
    );
    if (response.length) {
...
export const processSendEmailLink = async (
  ...
  try {
    ...
    const result = await customerModel.handleSendEmailLink(
      signUpToken,
      email
    );
    return res.sendStatus(200);
  ...
  try {
    ...
    try {
      const response: number =
        await customerModel.handlesCustLastViewdCat(
          cat_id,
          customer_id
        );
