...
import { UserInfo } from "../interfaces/interfaces.js";
import { Request, Response, NextFunction } from "express";
import * as sellerModel from "../model/seller.model.js";
...
export const processGetAllProductsOfSeller = async (
  ...
  try {
    ...
    const response: JSON = await sellerModel.handleGetAllProducts(
      sellerId
    );
...
export const processLogin = async (
  ...
  if (!email || !password) {
    ...
    try {
      const response: UserInfo | null = await sellerModel.handleLogin(
        email,
        password
      );
      if (response) {
...
export const processVerifyOTP = async (
  ...
  try {
    ...
    const response = await sellerModel.handleVerifyOTP(
      seller_id,
      OTP
    );
    if (response.length) {
...
export const processSendEmailLink = async (
  ...
  try {
    ...
    const result = await sellerModel.handleSendEmailLink(
      signUpToken,
      email
    );
    return res.sendStatus(200);
...
export const processSignUp = async (
  ...
  try {
    ...
    if (!shopName || !password || !email || !phone_number)
      return res.sendStatus(400);
    const response = await sellerModel.handleSignUp(
      shopName,
      password,
      email,
      phone_number
