...
export const handlesGetProductDetails: any = async (
  productId: number
) => {
...
export const handlesGetCartDetails: any = async (
  customerId: number
) => {
  const promisePool = pool.promise();
  const connection = await promisePool.getConnection();
...
export const handlesGetRecommendedProductsBasedOnCat: any = async (
  category_id: number
) => {
  const promisePool = pool.promise();
  const connection = await promisePool.getConnection();
...
export const handlesGetWishlistItems: any = async (
  customer_id: number
) => {
  const promisePool = pool.promise();
  const connection = await promisePool.getConnection();
...
export const handlesTopProducts: any = async () => {
  const promisePool = pool.promise();
  const connection = await promisePool.getConnection();
...
export const handlesSearchBarPredictions: any = async () => {
  const promisePool = pool.promise();
  const connection = await promisePool.getConnection();
...
export const handlesSearchResult: any = async (input: string) => {
  const promisePool = pool.promise();
  const connection = await promisePool.getConnection();
  const sql = `SELECT products.product_id, products.name 
  FROM listed_products
...
export const handlesProductsBasedOnCategory: any = async (
  category_id: number
) => {
  const promisePool = pool.promise();
  const connection = await promisePool.getConnection();
...
export const handlesInsertingWishlistedProduct: any = async (
  customer_id: number,
  product_id: number
) => {
  const promisePool = pool.promise();
